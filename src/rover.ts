...
export function toRoverInstruction(
  instruction: string
): RoverInstruction | null {
...
export class Rover {
  ...
  validatePosition = (): boolean => {
    return this.x_Ordinate <= this.grid.width &&
      this.x_Ordinate >= 0 &&
      this.y_Ordinate <= this.grid.height &&
      this.y_Ordinate >= 0
      ? true
      : false;
  };
  ...
  executeRoverInstruction = (instruction: "L" | "R" | "M") => {
    ...
    for (
      let i = 0;
      i < instructionsArray.length;
      i++ //Looping through each element to validate the position first and then moving
    ) {
      ...
      } else throw "Rover's attempting to go beyond the grid boundary";
    }
  };
